   ...
   # relative object location
    ...
    def get_next_instruction(self):
        ...
        if len(self.checkpoints[0]) == 1: # last action in set
            ...
            self.checkpoints.pop(0)
            print("Last action in set i have arrived")
            #add code to creep forward if target is too far away
        ...
        if len(self.queued_actions) == 0: # if self.queued_actions is empty
            ...
            if action_type == 1:    # Target heading
                ...
                #detection = self.detector_output[0]
                    print(detection)
                    obj_pose = self.estimate_pose(self.camera_matrix,detection,self.get_robot_state())
                    if obj_pose:
                        print(obj_pose)
                        x_pose = obj_pose['x']
                        y_pose = obj_pose['y']
                        detected_fruit = detection[0]
                        ...
                        if len(indexs) > 1:
                            print('Multiple fruits detected')
                        final = [detected_pose, indexs]
                        print(final)
    ...
    def navigate_to_fruits(self):
        """
        Calculates the path to the first fruit on the shopping list.
        """
        initial_pos = [0,0]
        ...
        # Get coordinates of the first fruit on the shopping list    
        waypointList = []
        if self.search_list:
            waypointList.append(fruit_true_pos[self.fruit_list.index(self.search_list[0])])

        safe_radiuses = [0.35,0.3,0.25,0.2,0.19,0.18,0.17,0.16,0.15,0.14,0.13,0.12,0.11,0.1] #revert to normal if takes too long
        bounds = {}

        # Plan route to the first waypoint
        if waypointList:
            ...
            for radius in safe_radiuses:                
                ...
                for _ in range(50):
                    ...
                    path = rrtc.planning(animation=False)
                    if path:
                        routes.append(path)
                if routes:
                    self.checkpoints = min(routes, key=len)
                    break

    def update_path_to_next_target(self):
        """
        Updates the path to the next fruit on the shopping list.
        """
        ...
        # Get coordinates of the next fruit on the shopping list    
        waypointList = []
        if self.search_list:
            waypointList.append(fruit_true_pos[self.fruit_list.index(self.search_list[0])])
    ...
    # Plan route to the next waypoint
        if waypointList:
            waypoint = waypointList[0]
            print("navigating to", waypoint)
            tempObstacleCoorList = FullObstacleCoorList.copy()
        ...
        # Remove the reached target from the search list
        if self.search_list:
    ...
    def update_map_and_recalculate_path(self, detections):
        ...
        for detection in detections:
            detected_pose = detection[0]
            fruit_indices = detection[1]

            # duplicate fruit detected
            if len(fruit_indices) > 1:
                # Find the closest fruit index
                closest_index = None
                ...
                for fruit_index in fruit_indices:
                    ...
                    if distance < closest_distance:
                        ...
                        closest_index = fruit_index

                fruit_index = closest_index
            else:
                fruit_index = fruit_indices[0]
...
if __name__ == "__main__":
    ...
    while start and operate.completed is False:
        ...
        if len(operate.checkpoints) == 0:
            operate.update_path_to_next_target()
        else:
            break