...
class Robot:
    ...
    def drive_with_v_w(self, linear_velocity, angular_velocity, dt):
        # left_speed and right_speed are the speeds in ticks/s of the left and right wheels.
        # dt is the length of time to drive for
    ...
    def covariance_drive(self, drive_meas):
        ...
        if ang_vel != 0:
            ...
            df1dw = (lin_vel / ang_vel**2)*(np.sin(th)+dt*ang_vel*np.cos(dt*ang_vel + th)-np.sin(dt*ang_vel + th)) # af
            df1dv = (1/ang_vel) * (-np.sin(th) + np.sin(th + ang_vel*dt)) # af
            df2dw = (lin_vel / ang_vel**2)*(dt*ang_vel*np.sin(dt*ang_vel + th) + np.cos(dt*ang_vel + th) - np.cos(th)) # af 
            df2dv = (1/ang_vel) * (np.cos(th) - np.cos(th + ang_vel*dt))
            df3dv = 0
            df3dw = dt
            
            # OLD
            #df1dw = (-lin_vel / ang_vel**2)*(np.sin(th)) + (lin_vel/ang_vel**2)*((ang_vel*np.cos(th + dt*ang_vel)*dt) - (np.sin(th + dt*ang_vel)))
            #df2dw = (lin_vel / ang_vel**2)*(np.cos(th)) + (lin_vel/ang_vel**2)*((ang_vel*np.sin(th + dt*ang_vel)*dt) + (np.cos(th + dt*ang_vel)))
            #df1dv = (-np.sin(th)/ang_vel) + (np.sin(th + ang_vel*dt))
            #df2dv = (np.cos(th)/ang_vel) - (np.cos(th + ang_vel*dt))
