...
class wp_manager:
    ...
    def __init__(self):
        ...
        self.target_fruit_detected = False  # Flag to indicate if the target fruit is detected
    ...
    def tick(self, robot, b_path_found, b_saw_lm_last_tick, lms):
        ...
        if self.state == "PauseAfterTargetReached":
            if time.time() - self.pause_timer < 3.0:
                print('paused')
                return [0,0]
            else:
                print('Pause complete, resetting state')
                self.distance_to_target = float('inf')
                self.target_fruit_detected = False
        ...
        # Pause after End
        if self.arrived:
                    # Rotate until the target fruit is detected
            #comment out if doesn't work
            ################################################################################################
            if not self.target_fruit_detected:
                self.state = "Searching for target fruit"
                print('Searching for target')
                return [0, 2]  # Rotate in place
            else:
                self.state = "Target fruit detected"
                #time.sleep(1)
                print('Target fruit detected, stopping rotation')
                print('Distance to target fruit:', self.distance_to_target)
                #self.target_fruit_detected = False  # Reset target fruit detection
                if self.distance_to_target == float('inf'):
                    return [0, 0]
                
                # Move towards the target fruit until within 20 cm
                if self.distance_to_target > 0.3:
                    # If the target fruit is lost while moving, rotate to search for it again
                    if not self.target_fruit_detected:
                        self.state = "Lost sight of target fruit, rotating to search"
                        print('Lost sight of target fruit, rotating to search')
                        return [0, 2]  # Rotate in place
                    return [self.K_pv, 0]  # Move forward
                else:
                    print('Reached target fruit, pausing for 3 seconds')
                    self.state = "PauseAfterTargetReached"
        ...
        if (dist_from_final_goal < 0.22) or (self.finished_waiting):
