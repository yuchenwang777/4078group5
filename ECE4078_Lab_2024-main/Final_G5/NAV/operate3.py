...
from waypoint_manager2 import wp_manager
...
class Operate:
        ...
        if self.b_auto_fruit_search:
            if self.detector_output:
                ...
                for detection in self.detector_output:
                    ...
                    target_fruit_name = self.get_current_target_fruit_name()
                    if detection[0] == target_fruit_name and self.wp_manager.arrived:
                        self.wp_manager.target_fruit_detected = True
                        target_pose, relative_pose = self.estimate(self.ekf.robot.camera_matrix, detection, self.ekf.robot.state)
                        print(relative_pose)
                        if relative_pose:
                            self.wp_manager.distance_to_target = max(relative_pose['x'], relative_pose['y'])
                        break  # Exit the loop once the target fruit is detected

    def estimate(self, camera_matrix, obj_info, robot_pose):
        """e
        function:
    ...
    # TODO: measure actual sizes of targets [width, depth, height] and update the dictionary of true target dimensions
        target_dimensions_dict = {'pear': [77/1000,71/1000,85/1000], 'lemon': [0.074,0.05,0.06], 
                              'lime': [0.07,0.05,0.055], 'tomato': [0.07,0.065,0.069], 
   ...
   # relative object location
    ...
    def get_current_target_fruit_name(self):
        # Get the current target fruit index
        current_index = self.shopping_manager.grocery_idx
        # Get the fruit name from the shopping list using the index
        current_target_fruit_name = self.shopping_manager.shopping_list[current_index][0]
        return current_target_fruit_name
    ...
    def perform_auto_fruit_search(self):
        if self.b_auto_fruit_search:
            ...
            if self.wp_manager.completed_route and self.shopping_manager.is_not_finished():
                time.sleep(2) # stop when fruit found
    ...
    def draw(self, canvas):
        ...
        overlay_image = pygame.transform.scale(overlay_image, (100, 100))
